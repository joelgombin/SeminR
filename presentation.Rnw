...
<<union, dev=pdf, out.width=.4\linewidth, fig.align=center, fig.path=images/>>=
require(gpclib)
gpclibPermit()
require(maptools)
...
<<carteprop,dev=pdf, out.width=.55\linewidth, fig.align=center, message=FALSE, fig.path=images/, resulats='markup'>>=
somme <- picardie[picardie@data$CODE_DEPT == "80",]
carte.prop(somme, mini_picardie, varname="AbsIns", 
           sp.key="codeINSEE", data.key="codeINSEE", 
           at=quantile(mini_picardie$AbsIns, c(0,.25,.5,.75,1)))
@
